...
# 记忆搜索相似度阈值（0-1，低于此值的记忆不使用）
MEM0_SIMILARITY_THRESHOLD = 0.5

# 记忆注入到对话的提示词模板
# {memories}: 搜索到的相关记忆，每条一行
# {user_name}: 当前用户名（如已识别）
MEM0_CONTEXT_TEMPLATE = """以下是关于当前用户的一些已知信息，请在回答时自然地参考这些信息：

{memories}

请注意：
1. 如果上述信息与当前问题相关，自然地融入回答中
2. 不要直接说"根据记录"或"我记得"等生硬的表述
3. 如果信息与当前问题无关，可以忽略"""

# 关键信息提取提示词（用于从对话中提取需要记忆的内容）
MEM0_EXTRACTION_PROMPT = """请从以下对话中提取需要长期记忆的关键信息。只提取以下类型的信息：
1. 人物喜好（如：喜欢/不喜欢的食物、颜色、活动等）
2. 人物关系（如：家人、朋友、同事等）
3. 重要事实（如：生日、职业、住址等）
4. 用户明确要求记住的内容
...
请以简洁的陈述句形式输出需要记忆的内容，每条一行。如果没有需要记忆的内容，输出"无"。
只输出提取的信息，不要有任何解释。"""
